...
#include <cassert>
#include <set> // Needed as CLion claims set is not in std

#include <SFML/Window.hpp>
#include <vulkan/vk_platform.h>

export module vkt.Application;
...
import :QueueFamilies;
import :SwapChainDetails;

[[nodiscard]] int rate_surface_format(const vk::SurfaceFormatKHR &surface_format) {
    int total = 0;
...
export class Application {
  ...
  private:
    ...
    debugCallback([[maybe_unused]] vk::DebugUtilsMessageSeverityFlagBitsEXT message_severity,
    ...
    [[nodiscard]] std::vector<std::string_view> selectLayers() const {
        ...
        for (auto &requested_layer : requested_layers) {
            ...
            for (auto &layer : layers) {
                if (static_cast<std::string_view>(layer.layerName.data()) ==
    ...
    void createImageViews() {
        ...
        constexpr vk::ImageSubresourceRange subresource_range(vk::ImageAspectFlagBits::eColor, 0, 1,
                                                              0, 1);

        for (const auto &image : swapChainImages) {
            vk::ImageViewCreateInfo create_info({}, image, vk::ImageViewType::e2D,
                                                swapChainImageFormat, {}, subresource_range);
    ...
    void createRenderPass() {
        ...
        constexpr vk::AttachmentReference color_attachment(
...
#include "triangle.spv.h"
        ...
        vk::PipelineShaderStageCreateInfo fragment_stage_info(
            {}, vk::ShaderStageFlagBits::eFragment, shader_module, "fragmentMain");
    ...
    void recordCommandBuffer(const vk::raii::CommandBuffer &command_buffer,
        ...
        const vk::RenderPassBeginInfo render_pass_info(
            renderPass, swapChainFramebuffers[image_index], vk::Rect2D({}, swapChainExtent),
            clear_color_value);
